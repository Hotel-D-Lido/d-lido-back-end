...
module.exports = class AuthenticationController {
  async login (req, res) {
    ...
    // const obtenerListado = await Guest.findOne(email)

    // if (guest) {
    //   window.localStorage.setItem('persona', guest)
    // }

    if (guest) { return res.status(200).send({ message: 'Accediendo' }) }
    ...
    if (match) { return res.status(200).send({ message: 'Accediendo' }) }
